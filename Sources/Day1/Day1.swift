...
struct DayOne {
  ...
  static let lines = input
    .split(separator: "\n")
  
  static let components = lines
    .map { $0.split(separator: "   ") }
  
  static let listOne: [Int] = components.lazy
    .map(\.[0])
    .map(String.init)
    .compactMap(Int.init)
  
  static let listTwo: [Int] = components.lazy
    .map(\.[1])
    .map(String.init)
    .compactMap(Int.init)
  
  static let countsTwo: [Int: Int] = listTwo
    .reduce(into: [Int: Int]()) { counts, item in counts[item, default: 0] += 1 }
  ...
  static var partOne: Int {
    zip(listOne.sorted(), listTwo.sorted())
      .reduce(into: 0) { partialResult, pair in
        partialResult += abs(pair.1 - pair.0)
      }
  }
  ...
  static var partTwo: Int {
    listOne.reduce(0) { count, number in
      count + number * (countsTwo[number] ?? 0)
    }
  }
